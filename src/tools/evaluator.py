from tools import modes, plotter
...
def get_lswm_improvement(instructions: list[instruction_model.Instruction], base_isnt: str, new_byte_count: int, base_regs, dest_regs):
    '''
    ...
    for inst in instructions[1:]:
        ...
        is_mem_pair = (is_eq 
                       and is_mem 
                       and is_base_reg 
                       and is_last_base_reg 
                       and is_dest_reg 
                       and is_last_dest_reg 
                       and abs(abs(inst.get_imm().value) - abs(last.get_imm().value)) == 4 
                       and inst.regs[0] != last.regs[0])
        if is_mem_pair:
            # chain detected
...
def get_byte_count(instructions: list[instruction_model.Instruction]) -> int:
    result = 0
    for inst in instructions:
...
def get_cnt_byte_count(opcode_inst_cnt_map) -> int:
    result = 0
    for op in opcode_inst_cnt_map.keys():
        inst, cnt = opcode_inst_cnt_map[op]
        result += inst.get_size()*cnt
    return int(result)
...
def rel(count, total):
    if total != 0:
        ...
        print("ERROR: cannot calculate percent with a total of zero")
        return 0


def plot_individual_static_stats(file, instructions, tp, path):
...
def print_individual_static_improvement(file, instructions):
...
def print_total_static_improvement(total):
    ...
    # x*2 is the count of Bytes saved by a reduction to 16 bit inst
    improvement = get_improvement(stats, lambda x: x*2)
    print('  Total Improvement by replacing 32 with 16 Bit inst: ' + str(improvement) + ' Byte ==', round(rel(improvement, total_byte_count)), '%')

    pairs = most_pairs(total, 1, equal=False, connected=True)
    # x contains count of 16 or 32 Bit instructions pairs
    # x*6 is the count of Bytes saved by a reduction to 16 bit inst
    improvement = get_improvement(pairs, lambda x: x*6)
...
def plot_individual_dynamic_stats(file, instructions, tp, path):
    for mode in modes.Mode:
            stats = most_inst(instructions, mode, modes.SearchKey.MNEMONIC, 10)
    ...
    lw_dist = inst_vals(instructions, 'lw', 10)
...
def print_individual_dynamic_improvement(file, instructions):
    stats = most_inst(instructions, modes.Mode.FULL, modes.SearchKey.MNEMONIC, 10000000)
    # x contains count of 32 Bit (4 Byte) instructions
    # x*2 is the count of Bytes saved by a reduction to 16 bit inst
    improvement = get_improvement(stats, lambda x: x*2)
...
def print_total_dynamic_improvement(total):
