...
def plot_bars(stats, filename: str, tp: str, path: str, mode: modes.Mode, search_key: modes.SearchKey, tex: bool=False):
    # set width of bars
    ...
    if tex:
        ...
        with open(fig_name + '.tex', 'w') as f:
            for r in range(len(mnemonics)):
                f.write(str(mnemonics[r]) + ': ' + str(counts[r]) + '\n')
    plt.close()
