...
if [ -z "$RISCV_TOOLCHAIN_PATH_CHANGED" ]; then
...
CC="${RISCV_TOOLCHAIN_PATH_CHANGED}/bin/clang"
CFLAGS="-D__riscv__ -Oz -march=rv32imacfdk -mabi=ilp32 -S  -masm=intel  -fno-builtin-bcmp -I${SUPPORT_PATH} -I${EMBENCH_PATH}/config/riscv32/boards/ri5cyverilator -I${EMBENCH_PATH}/config/riscv32/chips/generic -I${EMBENCH_PATH}/config/riscv32 -DCPU_MHZ=1 -DWARMUP_HEAT=1"
...
cd ${BUILD_PATH}
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE=rv32imacfd-toolchain.cmake -DRISCV_TOOLCHAIN_PREFIX=${RISCV_TOOLCHAIN_PATH_CHANGED} -DCMAKE_INSTALL_PREFIX=../install ..
make VERBOSE=1 -j8
make install
